...
class WeatherApiTest {
    ...
    void setUp() {
        ...
        api = new WeatherApi(factoryMock, JsonbBuilder.create());
    }
    ...
    void shouldAttemptToConnectToRightUrl() {
        //when
        when(connectionMock.response()).thenReturn(SAMPLE_RESPONSE);
        api.weatherbitResponse(City.Jastarnia);

        //then
        verify(factoryMock).build("http://api.weatherbit.io/v2.0/forecast/daily?&city=Jastarnia&key=3a592d44641446cd87518392b3497ebb");
    }

    @Test
    void shouldReturnValidTemp() {
        //When
        when(connectionMock.response()).thenReturn(SAMPLE_RESPONSE);
        WeatherbitResponse result = api.weatherbitResponse(City.Jastarnia);

        //then
        assertThat(result.getWeatherbitResponseDetail().get(5).getTemp()).isEqualTo(2.9f);
    }
